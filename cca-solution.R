...
cca <- "
  ...
  vector<lower=0>[K] t_alpha;
	...
	model {
		...
		// stack columns of Z to a vector -- K dimensional standard normal multivariate is equivalent to sampling from k univariate standard normal distributions.
		// stack columns of alpha to a vector to sample from gamma
		for(m in 1:M) tau[m] ~ gamma(1,1);
